            ...
            when (status.statusCode) {
                CommonStatusCodes.SUCCESS -> {
                    ...
                    if (otpReceiver != null) {
                        ...
                        val otpString: Array<String> = otp.split("\\.".toRegex()).toTypedArray()
                        val splitString = otpString[0].split(" ".toRegex()).toTypedArray()

                        otpReceiver!!.onOTPReceived(splitString[splitString.size - 1])
                    }
                }
